...
QUARANTINE_ROOT = Path.home() / ".poe/skill-cleaner/quarantine"
...
def command_scan(args: argparse.Namespace) -> int:
    ...
    skills = [path for path in skills if path.name != "poe-skill-cleaner"]
...
def command_quarantine(args: argparse.Namespace) -> int:
    ...
    if not (skill_dir / "SKILL.md").is_file() or skill_dir.name == "poe-skill-cleaner":
        print("拒绝执行：目标必须是非 poe-skill-cleaner 的 skill 目录。", file=sys.stderr); return 2
