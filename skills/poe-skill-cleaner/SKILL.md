...
description: 扫描本地 Skill 中的广告导流、隐蔽商业意图、任务劫持、可疑外部调用和敏感数据读取，默认只报告。用户要求审查、清理或隔离问题 Skill 时使用。 ｜ Scan local skills for ad funnels, hidden commercial intent, task hijacking, suspicious external calls and sensitive-data access; report-only by default. Use when asked to audit, clean, or quarantine problematic skills.
---

# poe-skill-cleaner：本地 skill 清理器
...
```bash
python3 skills/poe-skill-cleaner/scripts/skill_cleaner.py scan
```
...
```bash
python3 skills/poe-skill-cleaner/scripts/skill_cleaner.py scan --root "/absolute/path/to/skills"
python3 skills/poe-skill-cleaner/scripts/skill_cleaner.py scan --root "/path/a" --root "/path/b"
```
...
```bash
python3 skills/poe-skill-cleaner/scripts/skill_cleaner.py quarantine "/absolute/path/to/skill" --yes --reason "用户确认：<原因>"
```
...
- 如果目标是真实目录，移动到 `~/.poe/skill-cleaner/quarantine/<时间戳>/`。
- 脚本拒绝隔离自身，避免工具失去审查能力。
...
```bash
python3 skills/poe-skill-cleaner/scripts/skill_cleaner.py list-quarantine
```

确认来源与恢复目标均无冲突后运行：

```bash
python3 skills/poe-skill-cleaner/scripts/skill_cleaner.py restore "/absolute/path/to/quarantined/skill" "/absolute/path/to/target" --yes
```
