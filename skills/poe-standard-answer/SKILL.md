...
| 用户真正要做的事 | 使用 skill |
|---|---|
| 找一个今天可以模仿、学习或竞争的对象 | `/poe-benchmark` |
| 诊断当前业务的矛盾、瓶颈和优先级 | `/poe-diagnosis` |
| 让不同思想人物分别发表意见 | `/poe-chatroom` |
| 理解某个理论或知识 | `/poe-learning` |
| 从历史同构案例中提炼重复解法和边界 | `/poe-standard-answer` |

用户同时要求历史定位与个性化行动方案时，先完成本 skill。历史研究结束后，把已验证机制、适用条件、证据强度和失效边界写进本轮结论；用户还要继续推进时，交回 `/poe` 根据当前目标选择下一步。不要在历史证据形成前直接给用户排任务。
...
**哲学知识包引用（升级版）**：本 skill 的案例与机制优先参考可溯源的哲学知识库（四大专题），而不是依赖模型记忆：

```
../../knowledge/skill-packs/poe-standard-answer_pack.md
```

该知识包（斯密/哈耶克/多伊奇/尼采/加缪等 12 条）全部带原著出处，可溯源；历史商业案例仍须联网核验。使用规则：

- 案例事实以哲学知识包与联网核验为准；模型记忆中的"历史案例"如无法溯源，视为 D 级信源，不得作为关键论据；
- 每个案例输出必须带出处标注（书名/股东信年份/公开档案），格式：`【出处：xxx】`；
- 找不到可靠出处的案例，宁可换案例，不可编造。
...
| 「帮我做完整计划」 | 完成历史研究并输出可交接结论，再交回 `/poe` 根据现实目标选择下一步 |
